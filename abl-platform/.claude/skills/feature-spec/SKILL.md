...
5. **Spawn the product-oracle agent** to answer these questions autonomously:
   ...
   - Include: "Answer these clarifying questions for the feature: <feature-name>. Read docs/features/, CLAUDE.md, and relevant code."
...
### Phase 4b: Audit Suite (5 passes)

After generating the feature spec, run 5 audit passes. Passes 1-2 check spec quality; passes 3-5 check against external context and can run in parallel after passes 1-2 are resolved.

#### Passes 1-2: Spec Quality (sequential)
...
3. If APPROVED on round 1: still run round 2 as a fresh-eyes pass (focuses on cross-phase consistency)
4. After round 2: proceed to passes 3-5 regardless (log remaining MEDIUM findings)

**Ralph-loop integration:** If running inside a ralph-loop, each loop iteration counts as an audit round. Do NOT run internal audit loops redundantly with the ralph-loop. Check for `.claude/ralph-loop.local.md` to detect if you're in a loop.

#### Passes 3-5: External Context Audits (run in parallel after passes 1-2)

Spawn all three agents in a single message:

**Pass 3 — Platform Audit** (general-purpose agent, reads local files):

```
Platform audit for feature spec at: docs/features/<slug>.md

Read the spec, then check:
1. Read CLAUDE.md — does the design contradict any core invariants? (tenant isolation,
   centralized auth, stateless distributed, traceability, compliance, user isolation rules)
2. Read .claude/skills/platform-principles/SKILL.md — does it respect established patterns?
3. Grep the codebase for existing capabilities in the same domain — is the spec reinventing
   something already present? Look for duplicate MongoDB collections, duplicate Redis key
   patterns, duplicate service abstractions.
4. Does the proposed API surface follow the project's route naming conventions, error envelope
   format ({ success, data?, error? }), and ownership scoping rules?
5. Does the data model respect the project's tenant/project/user isolation layers?

Return: APPROVED or NEEDS_REVISION. For each finding: [CRITICAL|HIGH|MEDIUM] + exact
spec section to fix + recommended change.
```

**Pass 4 — Industry Research Expert Audit** (general-purpose agent, uses WebSearch/WebFetch):

```
Industry research audit for feature spec at: docs/features/<slug>.md

Read the spec first. Then research:
1. What are the industry-standard patterns for this feature domain? Find at least 2
   authoritative sources (RFCs, OWASP guides, well-known engineering blogs, standards bodies).
2. Find 2-3 comparable implementations in production SaaS products or well-maintained OSS
   projects. How do their design decisions compare to what this spec proposes?
3. What are the known failure modes, security risks, or scaling problems for this class of
   feature? Does the spec address them?
4. Are there established metrics or success criteria used in the industry for this feature type
   that the spec's Success Metrics section is missing?
5. What does industry experience say about the spec's proposed approach — is it mainstream,
   experimental, or deprecated?

Return: list of findings tagged [IMPROVEMENT], [RISK], or [GAP] with specific spec section
references and the source that informed each finding.
```

**Pass 5 — OSS Library Audit** (general-purpose agent, uses WebSearch/WebFetch):

```
Open-source library audit for feature spec at: docs/features/<slug>.md

Read the spec first. Then research:
1. For each major custom-implementation proposed in the spec, search for existing OSS libraries
   that implement the same capability.
2. For each candidate library: name, license (MIT/Apache preferred — flag GPL as incompatible
   with commercial SaaS), last release date, weekly downloads or GitHub stars, maintenance
   status (active/maintained/abandoned).
3. Rank candidates: (a) adopt as-is, (b) vendor/fork, (c) use for design inspiration only,
   (d) avoid (GPL, abandoned, or poor quality).
4. Flag any spec sections proposing custom implementations that a well-maintained OSS library
   could replace with lower risk.
5. If a library is adopted, note any migration or integration cost.

Return: OSS candidates table (name | license | stars | status | recommendation) + one
sentence per item on why.
```

**Integrate findings after all three pass 3-5 agents return:**

1. Fix any CRITICAL or HIGH **platform-audit** findings before proceeding — these are blocking
2. Log **industry research** findings to the spec's Open Questions or Gaps section; promote key insights into Functional Requirements or Non-Functional Concerns as needed
3. Log **OSS library** recommendations to the spec's Implementation Files / Dependencies section; if a library should replace proposed custom code, update the spec's Implementation Approach accordingly
4. Platform-audit MEDIUM findings, and all industry/OSS findings, are non-blocking but must be logged

**Log all 5 pass findings** to `docs/sdlc-logs/<slug>/feature-spec.log.md` — include pass number, verdict, findings, and resolutions.
