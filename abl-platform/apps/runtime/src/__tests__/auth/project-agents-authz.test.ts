/**
 * Project Agents Authorization Tests — Project-Level Object:Operation RBAC
 *
 * Verifies that `requireProjectPermission` enforces project-level permissions
 * on the project-agents router using object:operation format.
 *
 ...
 *
 * Permission mapping:
 *   GET  /                → agent:read    (all project members)
 *   GET  /:agentName      → agent:read    (all project members)
 *   PUT  /:agentName/dsl  → agent:update  (admin, developer — not viewer)
 *
 * Project role → permissions:
 *   admin     → *:* (all)
 *   developer → agent:* (full agent access — read + update)
...
vi.mock('../../repos/project-repo.js', () => ({
  ...
  updateProjectAgentDsl: vi.fn().mockResolvedValue({
    id: 'agent-1',
    updatedAt: new Date(),
  }),
}));
...
vi.mock('../../services/version-service.js', () => ({
  ...
  safeParseJSON: vi.fn((val: any, def: any) => {
...
const { default: projectAgentsRouter } = await import('../../routes/project-agents.js');
...
const UPDATE_DSL_BODY = { dslContent: 'agent main { goal "test" }' };

// =============================================================================
// TESTS
// =============================================================================

describe('Project Agents route authorization — project-level object:operation RBAC', () => {
  // ---------------------------------------------------------------------------
  ...
  describe('Tenant OWNER (workspace authority)', () => {
    ...
    test('GET / passes (agent:read — workspace authority)', async () => {
      const { status } = await request(baseUrl, 'GET', AGENTS_BASE);
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });

    test('GET /:agentName passes (agent:read — workspace authority)', async () => {
      const { status } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });

    test('PUT /:agentName/dsl passes (agent:update — workspace authority)', async () => {
      const { status } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });
  ...
  describe('Project owner (ownerId match)', () => {
    ...
    test('GET /:agentName passes (agent:read — project owner)', async () => {
      const { status } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });

    test('PUT /:agentName/dsl passes (agent:update — project owner)', async () => {
      const { status } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
      expect(status).not.toBe(403);
  ...
  describe('Project admin member', () => {
    ...
    test('GET /:agentName passes (admin has *:* → agent:read)', async () => {
      const { status } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });

    test('PUT /:agentName/dsl passes (admin has *:* → agent:update)', async () => {
      const { status } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
      expect(status).not.toBe(403);
  ...
  describe('Project developer member', () => {
    ...
    test('GET /:agentName passes (developer has agent:* → agent:read)', async () => {
      const { status } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });

    test('PUT /:agentName/dsl passes (developer has agent:* → agent:update)', async () => {
      const { status } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
      expect(status).not.toBe(403);
  ...
  describe('Project viewer member', () => {
    ...
    test('GET /:agentName passes (viewer has agent:read)', async () => {
      const { status } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).not.toBe(403);
      expect(status).not.toBe(401);
    });

    test('PUT /:agentName/dsl returns 403 (viewer lacks agent:update)', async () => {
      const { status, body } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
  ...
  describe('Non-member (OPERATOR without project membership)', () => {
    ...
    test('GET / returns 404 (project existence concealed from non-members)', async () => {
      const { status, body } = await request(baseUrl, 'GET', AGENTS_BASE);
      expect(status).toBe(404);
      expect(body.error).toMatchObject({ message: 'Project not found' });
    });

    test('GET /:agentName returns 404 (project existence concealed from non-members)', async () => {
      const { status, body } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).toBe(404);
      expect(body.error).toMatchObject({ message: 'Project not found' });
    });

    test('PUT /:agentName/dsl returns 404 (project existence concealed from non-members)', async () => {
      const { status, body } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
      expect(status).toBe(404);
  ...
  describe('Unauthenticated requests', () => {
    ...
    test('GET /:agentName returns 401', async () => {
      const { status, body } = await request(baseUrl, 'GET', `${AGENTS_BASE}/main`);
      expect(status).toBe(401);
      expect(body.error).toMatchObject({ message: 'Authentication required' });
    });

    test('PUT /:agentName/dsl returns 401', async () => {
      const { status, body } = await request(baseUrl, 'PUT', `${AGENTS_BASE}/main/dsl`, {
        body: UPDATE_DSL_BODY,
      });
      expect(status).toBe(401);
