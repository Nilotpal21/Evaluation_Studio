/**
 * Unit tests for the runtime semver descending comparator.
 *
 * runtime re-exports `compareSemverDesc` from `@agent-platform/shared-kernel`
 * via `workflow-version-service`; this suite exercises runtime's import path,
 * with a parallel suite in workflow-engine for the engine's.
 */

import { describe, it, expect } from 'vitest';
import { compareSemverDesc } from '../services/workflow-version-service.js';

describe('compareSemverDesc (runtime)', () => {
  ...
  it('valid semvers sort before invalid strings, which sort before draft', () => {
    ...
    versions.sort(compareSemverDesc);
    // v0.10.0, v0.9.0 (valid) come first, then invalids (unordered among themselves),
    // then 'draft' last
