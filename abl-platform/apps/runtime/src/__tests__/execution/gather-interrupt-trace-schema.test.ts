...
AGENT: GatherInterruptDigressionTraceSchema
...
AGENT: GatherInterruptSubIntentTraceSchema
...
describe('gather interrupt trace schema', () => {
  ...
  it('validates digression trace payloads against the canonical schema', async () => {
    const session = executor.createSessionFromResolved(
      compileToResolvedAgent([DIGRESSION_TRACE_AGENT], 'GatherInterruptDigressionTraceSchema'),
    );
  ...
  it('validates sub-intent trace payloads against the canonical schema', async () => {
    const session = executor.createSessionFromResolved(
      compileToResolvedAgent([SUB_INTENT_TRACE_AGENT], 'GatherInterruptSubIntentTraceSchema'),
    );
    await executor.initializeSession(session.id);
  ...
  it('validates parent-supervisor reroute trace payloads against the canonical schema', async () => {
