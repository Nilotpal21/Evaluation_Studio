/**
 * Messenger Media Processor
 *
 * Orchestrates the download-then-upload flow for Messenger media attachments.
...
import type {
  MessengerMediaReference,
  MessengerMediaDownloadResult,
} from './messenger-media-downloader.js';
...
const log = createLogger('messenger-media-processor');

export type MessengerMediaReferenceMetadata = MessengerMediaReference;
...
export interface ProcessOptions {
  ...
  downloadFn: (ref: MessengerMediaReference) => Promise<MessengerMediaDownloadResult>;
  uploadFn: (params: {
    stream: Readable;
    filename: string;
    mimeType: string;
...
export async function processMessengerMediaReferences(
  mediaRefs: MessengerMediaReferenceMetadata[],
