/**
 * Template Store Entry Point
 *
 * Initializes database connection, then starts the Express server.
 * Follows the same startup pattern as runtime/src/index.ts.
 */
...
function resolveMongoDirectConnection(mongoUrl: string): boolean {
  const explicitValue = process.env.MONGODB_DIRECT_CONNECTION;
  if (explicitValue !== undefined) {
    return explicitValue === 'true';
  }

  try {
    return new URL(mongoUrl).searchParams.get('directConnection') === 'true';
  } catch {
    return false;
  }
}
...
async function main(): Promise<void> {
  try {
    ...
    log.info('Starting Template Store', { env: config.env, port: config.port });
    ...
    try {
      await initMongoBackend({
        ...
        retryWrites: true,
        retryReads: true,
        directConnection: resolveMongoDirectConnection(config.mongoUrl),
        autoIndex: process.env.NODE_ENV !== 'production',
        slowQueryThresholdMs: parseInt(process.env.MONGODB_SLOW_QUERY_MS || '200', 10),
        appName: 'abl-template-store',
      });
    ...
    log.error('Failed to start template store', {
      error: error instanceof Error ? error.message : String(error),
    });
    process.exit(1);
  }
