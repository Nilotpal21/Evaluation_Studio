...
For voice sessions, the runtime makes caller identity available through the `session` namespace. This lets an agent pass the caller number or called number into tools without asking the caller to repeat it.
...
The runtime normalizes phone-like values from the voice gateway. For example, `sip:+15551234567@example.com` and `+1 (555) 123-4567` both become `+15551234567`. If the first caller value is not a phone number, the runtime checks trusted SIP identity headers before falling back to the raw identity value.
...
Caller identity fields can contain phone numbers or carrier-provided identifiers, so treat them as end-user identity data. Do not pass raw fields to external tools unless that tool needs the original gateway value.
