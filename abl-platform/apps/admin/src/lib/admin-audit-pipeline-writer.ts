import { BufferedKafkaTopicPublisher } from '@abl/eventstore/queues/buffered-kafka-topic-publisher';
import { parsePositiveIntEnv } from '@abl/eventstore/queues/env-utils';
import {
...
const DEFAULT_AUDIT_KAFKA_CLIENT_ID = 'abl-admin-audit';
...
const GLOBAL_SHUTDOWN_HOOK_KEY = '__abl_admin_audit_pipeline_shutdown_hook__' as const;

function ensureAdminAuditShutdownHooksRegistered(): void {
  ...
  const flushOnShutdown = () => {
    void closeAdminAuditPipelineWriter();
  };
...
function getAdminAuditPublisher(
  ...
  ensureAdminAuditShutdownHooksRegistered();

  return publisher;
}

export function publishAdminAuditPipelineEvent(
  ...
  getAdminAuditPublisher(env).publish({
    key: tenantId ?? undefined,
    value: event,
  });
}

export async function closeAdminAuditPipelineWriter(): Promise<void> {
  if (!publisher) {
    return;
  }
