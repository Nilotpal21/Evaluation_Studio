...
import { addTenantMemberSchema } from '../../../../../lib/admin-proxy-schemas';
...
export const GET = withAdminRoute({ role: 'VIEWER' }, async (ctx: AdminRouteContext) => {
  ...
  try {
    const res = await fetch(
      `${getRuntimeBaseUrl()}/api/platform/admin/tenants/${encodeURIComponent(tenantId)}/members`,
...
export const POST = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  const { tenantId } = ctx.params;
  const parsedBody = await readValidatedJsonBody(ctx.request, addTenantMemberSchema);
