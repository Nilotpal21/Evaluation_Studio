...
import { tenantSubscriptionChangeSchema } from '../../../../../lib/admin-proxy-schemas';
...
export const PATCH = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  ...
  const parsedBody = await readValidatedJsonBody(ctx.request, tenantSubscriptionChangeSchema);
  ...
  try {
    const res = await fetch(
      `${getRuntimeBaseUrl()}/api/platform/admin/tenants/${tenantId}/subscription`,
      {
        method: 'PATCH',
