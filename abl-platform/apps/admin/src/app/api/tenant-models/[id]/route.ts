...
import { updateTenantModelSchema } from '../../../../lib/admin-proxy-schemas';
...
export const GET = withAdminRoute({ role: 'VIEWER' }, async (ctx: AdminRouteContext) => {
  ...
  try {
    const res = await fetch(
      `${getRuntimeBaseUrl()}/api/platform/admin/tenant-models/${encodeURIComponent(id)}`,
...
export const PATCH = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  const { id } = ctx.params;
  const parsedBody = await readValidatedJsonBody(ctx.request, updateTenantModelSchema);
...
export const DELETE = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  ...
  try {
    const res = await fetch(
      `${getRuntimeBaseUrl()}/api/platform/admin/tenant-models/${encodeURIComponent(id)}/revoke`,
      {
        method: 'POST',
        headers: buildRuntimeHeaders(ctx),
      },
