...
import { tenantModelProvisionSchema } from '../../../lib/admin-proxy-schemas';
...
export const GET = withAdminRoute({ role: 'VIEWER' }, async (ctx: AdminRouteContext) => {
  ...
  const url = `${getRuntimeBaseUrl()}/api/platform/admin/tenant-models${searchParams ? `?${searchParams}` : ''}`;
...
export const POST = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  const parsedBody = await readValidatedJsonBody(ctx.request, tenantModelProvisionSchema);
  if (!parsedBody.success) {
    return parsedBody.response;
  }

  try {
    const res = await fetch(`${getRuntimeBaseUrl()}/api/platform/admin/tenant-models`, {
      method: 'POST',
