...
import { createDealLineItemSchema } from '../../../../../lib/admin-proxy-schemas';
...
export const GET = withAdminRoute({ role: 'VIEWER' }, async (ctx: AdminRouteContext) => {
  ...
  const url = `${getRuntimeBaseUrl()}/api/platform/admin/deals/${encodeURIComponent(id)}/line-items${searchParams ? `?${searchParams}` : ''}`;
...
export const POST = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  const { id } = ctx.params;
  const parsedBody = await readValidatedJsonBody(ctx.request, createDealLineItemSchema);
  ...
  try {
    const res = await fetch(
      `${getRuntimeBaseUrl()}/api/platform/admin/deals/${encodeURIComponent(id)}/line-items`,
      {
        method: 'POST',
