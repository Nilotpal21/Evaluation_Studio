...
import { creditTopupSchema } from '../../../../../lib/admin-proxy-schemas';
...
export const GET = withAdminRoute({ role: 'VIEWER' }, async (ctx: AdminRouteContext) => {
  ...
  const url = `${getRuntimeBaseUrl()}/api/platform/admin/deals/${encodeURIComponent(id)}/credits${searchParams ? `?${searchParams}` : ''}`;
...
export const POST = withAdminRoute({ role: 'ADMIN' }, async (ctx: AdminRouteContext) => {
  const { id } = ctx.params;
  const parsedBody = await readValidatedJsonBody(ctx.request, creditTopupSchema);
  ...
  try {
    const res = await fetch(
      `${getRuntimeBaseUrl()}/api/platform/admin/deals/${encodeURIComponent(id)}/credits/topup`,
      {
        method: 'POST',
