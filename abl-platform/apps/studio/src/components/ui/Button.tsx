...
export type ButtonVariant = 'primary' | 'secondary' | 'ghost' | 'danger' | 'destructive-ghost';
...
const variantStyles: Record<ButtonVariant, string> = {
  ...
  secondary: 'bg-background text-foreground border border-default hover:bg-background-elevated',
  ghost: 'text-muted hover:text-foreground hover:bg-background-muted',
  danger: 'bg-error text-error-foreground hover:opacity-90',
  'destructive-ghost': 'text-error hover:text-error hover:bg-error/10',
};
  ...
  (
    ...
    return (
      ...
      >
        {loading ? (
          <svg
            className={clsx('animate-spin', spinnerSizeStyles[size])}
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
