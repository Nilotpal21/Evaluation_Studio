/**
 * POST /api/projects/:id/pipelines/:pipelineId/activate
 *
 * Thin proxy to the runtime activate endpoint.
 ...
 * response so the pipeline-engine picks up the status change immediately.
 */
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const res = await fetch(
      `${runtimeUrl}/api/projects/${projectId}/pipelines/${pipelineId}/activate`,
      {
        method: 'POST',
        headers: {
    ...
    return handleApiError(error, 'Pipeline Activate');
  }
}
