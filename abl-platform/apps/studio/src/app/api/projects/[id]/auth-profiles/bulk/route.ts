/**
 * POST /api/projects/:id/auth-profiles/bulk — Bulk actions on project-scoped auth profiles
 *
 * Supports: delete, revoke, activate
 * Each profile is verified for tenant + project ownership individually.
...
import { canWriteAuthProfile } from '@/app/api/auth-profiles/_auth-profile-route-utils';
import {
  executeBulkAction,
  loadModelMap,
  type BulkResult,
} from '../../../../auth-profiles/_bulk-handler';
...
// ─── POST — Bulk action (project-scoped) ────────────────────────────────
...
export const POST = withRouteHandler<BulkActionInput>(
  ...
  async ({ body, params, tenantId, user }) => {
    await ensureDb();
    ...
    for (const profileId of profileIds) {
      try {
        ...
        const result = await executeBulkAction({
          ...
          userId: user.id,
          isAdmin,
          ownershipFilter: { _id: profileId, tenantId, projectId },
          modelMap: models,
          AuthProfile,
        });
        results.push(result);
