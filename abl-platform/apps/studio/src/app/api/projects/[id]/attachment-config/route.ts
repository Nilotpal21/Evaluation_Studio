/**
 * GET/PUT /api/projects/:id/attachment-config — Proxy to runtime
 *
 * Proxies project attachment configuration to the runtime API.
 */
...
const log = createLogger('api:projects:attachment-config');
...
async function proxyToRuntime(request: NextRequest, { params }: RouteParams) {
  ...
  const runtimePath = `${getRuntimeUrl()}/api/projects/${projectId}/attachment-config`;
  ...
  try {
    ...
    return handleApiError(error, 'AttachmentConfig.proxy');
  }
}
