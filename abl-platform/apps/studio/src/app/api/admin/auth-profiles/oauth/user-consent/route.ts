/**
 * POST /api/admin/auth-profiles/oauth/user-consent
 *
 * Workspace-scoped (tenant-only) per-user OAuth consent capture. Mirrors the
 * project user-consent route but resolves profiles via the tenant-only lookup
 * helper and omits `projectId` from the Redis state payload.
 *
 * Added in ABLP-619 to remove the `projectId="_workspace"` sentinel previously
 * shipped through the project user-consent route.
 */
