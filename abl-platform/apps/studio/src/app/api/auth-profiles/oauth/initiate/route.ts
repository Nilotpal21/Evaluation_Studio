...
const InitiateWorkspaceSchema = z
  .object({
...
type InitiateWorkspaceInput = z.infer<typeof InitiateWorkspaceSchema>;
...
export const POST = withRouteHandler<InitiateWorkspaceInput>(
  {
    permissions: StudioPermission.AUTH_PROFILE_WRITE,
    bodySchema: InitiateWorkspaceSchema,
  ...
  async ({ body, user, tenantId, request }) => {
    ...
    if (typedProfile.usageMode === 'jit' || typedProfile.usageMode === 'preflight') {
      return errorJson(
        `OAuth initiation is not supported for usageMode '${typedProfile.usageMode}'.`,
        400,
        ErrorCode.VALIDATION_ERROR,
      );
    }

    const config = typedProfile.config as Record<string, unknown>;
    ...
    if (typeof resolvedAuthorizationUrl !== 'string' || resolvedAuthorizationUrl.length === 0) {
      return errorJson(
    ...
    const ssrfCheck = validateUrlForSSRF(resolvedAuthorizationUrl, getDevSSRFOptions());
    if (!ssrfCheck.safe) {
      return errorJson(
    ...
    const authUrl = new URL(resolvedAuthorizationUrl);
    ...
    const lockKey = `auth-profile:oauth-init-lock:${tenantId}:${String(typedProfile._id)}`;
    const lockResult = await redis.set(lockKey, '1', 'EX', 600, 'NX');
    if (lockResult !== 'OK') {
      return errorJson(
        'OAuth authorization is already in progress for this profile.',
        409,
    ...
    try {
      ...
      const resolvedVisibility =
        typedProfile.visibility === 'personal' ? ('personal' as const) : ('shared' as const);

      const { state, csrfNonce } = await createOAuthState(redis, {
        tenantId,
        projectId: null,
        userId: user.id,
        authProfileId: String(typedProfile._id),
        authProfileRef: body.authProfileRef ?? typedProfile.name,
        environment: body.environment ?? typedProfile.environment ?? null,
        authProfileScope: typedProfile.scope === 'tenant' ? 'tenant' : 'project',
        authProfileVisibility: resolvedVisibility,
        scopes: defaultScopes,
        isUserConsent: resolvedVisibility === 'personal',
        targetVisibility: resolvedVisibility,
        redirectUri,
        scope: 'workspace',
        ...((body.connectorName ?? typedProfile.connector)
          ? { connectorName: body.connectorName ?? typedProfile.connector }
          : {}),
      ...
      void emitOAuthAuditEvent({
        ...
        tenantId,
        userId: user.id,
        profileId: String(typedProfile._id),
        scope: 'workspace',
        projectId: null,
      });
      ...
      return response; // lock intentionally NOT released on success (600s TTL rate-limit)
    } catch (err) {
      // Release the lock so the user can retry immediately after a transient failure
