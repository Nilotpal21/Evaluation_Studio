...
import { querySessionTree } from '@/lib/arch-inspector-reader';

const log = createLogger('api:arch-ai:audit-logs:session-tree');
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rateLimit = await enforceArchAuditRateLimit(scope, 'session-tree');
    ...
    const events = await querySessionTree(scope.tenantId, sessionId, scope.projectId);

    return NextResponse.json({ success: true, events });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    log.error('Session tree error', { error: message });
