/**
 * GET /api/arch-ai/projects/:projectId/spec-document — Get spec document by project
 * PUT /api/arch-ai/projects/:projectId/spec-document — Bulk-update editable business fields (project-scoped)
 *
 * Project-scoped routes use `requireProjectAccess` instead of session owner checks.
 * PUT does NOT mirror to session metadata because the session is archived after
 * project creation.
 */
...
export async function GET(
  ...
  try {
    ...
    const specDoc = await specDocumentService.getByProject(
...
export async function PUT(
  ...
  try {
    ...
    // Get spec doc (project-scoped — no userId filter)
    ...
    // Project-scoped update: session is archived, no mirroring needed.
    // Build $set and apply directly to the spec doc (project-scoped filter).
    const specSet: Record<string, unknown> = {};
    for (const { path, value } of body.updates) {
      specSet[path] = value;
    }

    const updatedDoc = await ArchSpecDocument.findOneAndUpdate(
      { _id: spec._id, tenantId: auth.tenantId, projectId },
