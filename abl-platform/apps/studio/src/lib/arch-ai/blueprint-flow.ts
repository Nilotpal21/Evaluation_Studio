...
export function buildBlueprintConfirmWidget(summary?: string | null): BlueprintConfirmWidgetData {
  return {
    widgetType: 'BlueprintConfirm',
    question:
      'Would you like to turn this concept into a draft blueprint or refine the approach first?',
    title: 'Blueprint direction',
    description:
      summary?.trim() ||
      'We can lock in the architectural direction now or refine the concept before drafting the blueprint.',
    options: [
      {
        label: 'Generate draft blueprint',
        value: 'generate_draft_topology',
        description: 'Create the first blueprint document and agent graph.',
      },
      {
...
export function buildTopologyApprovalWidget(
  ...
  return {
    widgetType: 'TopologyApproval',
    question:
      'Review this draft blueprint. Accept it, request changes, or reject it and restart the concept.',
    title: 'Draft blueprint ready',
    description:
      summary?.trim() ||
      `This blueprint has ${agents.length} agents and ${topology.edges.length} handoffs.`,
...
export function buildTopologyRevisionWidget(
  ...
  return {
    widgetType: 'TopologyRevision',
    question: 'What should change in this draft blueprint?',
    title: 'Refine the draft blueprint',
    description:
      agentCount > 0
...
export function buildTopologyRevisionPrompt(
  ...
  if (mode === 'restart') {
    return notes
      ? `The user rejected the current draft blueprint and wants a different direction. Focus the redesign on ${targetText}. User notes: ${notes}. Explain the new architecture clearly in chat and regenerate a fresh draft blueprint.`
      : `The user rejected the current draft blueprint and wants a different direction. Focus the redesign on ${targetText}. Explain the new architecture clearly in chat and regenerate a fresh draft blueprint.`;
  }

  return notes
    ? `Revise the current draft blueprint. Focus on ${targetText}. User notes: ${notes}. Explain the revised architecture clearly in chat and regenerate the draft blueprint.`
    : `Revise the current draft blueprint. Focus on ${targetText}. Explain the revised architecture clearly in chat and regenerate the draft blueprint.`;
}
