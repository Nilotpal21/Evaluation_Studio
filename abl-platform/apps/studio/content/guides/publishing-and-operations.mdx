...
The preview shows agents that will be created, modified, or removed, plus any validation errors. The response also includes a root-level `previewDigest` you can log or compare before apply.
...
If apply fails after planning succeeds, the response now includes a staged error envelope with `error.stage` and `error.sanitizedCause` so automation can distinguish, for example, tool-write failures from locale-write failures without exposing raw internal error strings.
...
echo "Preview digest: $(echo $PREVIEW | jq -r '.previewDigest')"
...
- **Invalid locale path:** Locale assets must live at `locales/<locale>/<file>.json`. Preview rejects root-level files like `locales/en.json` or `locales/*.json` with `E_LOCALE_INVALID_PATH`.
...
- **Inline `TOOLS:` only, no `.tools.abl` file:** Preview shows the tool additions and apply auto-creates project-tool stubs. The next export will materialize those stubs under `tools/<name>.tools.abl`.
- **Rollback on failure:** If the import fails during application, newly created agents are automatically rolled back. Check `error.stage` and `error.sanitizedCause` before reviewing partial updates.
- **429 on import/apply:** Rate-limited responses include `Retry-After`; wait that many seconds before retrying.
