...
| `previewDigest` | string  | Stable digest for the preview payload, returned at the response root |
| `warnings`      | array   | Non-blocking import warnings                                         |
...
{
  "success": true,
  "previewDigest": "8f8d94e6f87d7e6f8e73f2f2",
  "preview": {
    "operations": [
      {
        "type": "update",
...
Preview also rejects invalid locale asset paths early. Locale files must live under `locales/<locale>/<file>.json`; paths such as `locales/en.json` or `locales/*.json` fail preview with `E_LOCALE_INVALID_PATH`.
...
| `entryAgentName` | string  | Entry agent selected after apply                                                             |
| `warnings`       | array   | Non-blocking import warnings                                                                 |
| `error`          | object  | Structured failure details. On apply failures this may include `stage` and `sanitizedCause`. |
...
If the import fails during apply, the error surface now includes the failing stage plus a sanitized persistence cause:
...
```json
{
  ...
  "error": {
    ...
    "stage": "create_tools",
    "sanitizedCause": "Duplicate record conflict"
  }
}
```
...
Inline agent `TOOLS:` signatures are previewed as tool additions and apply auto-creates project-tool stubs when the bundle does not include matching `.tools.abl` files.

Rate-limited import responses return `429` with a JSON error body and a `Retry-After` header.
