...
User-visible template strings also support a small filter set:

- `{{user.name | upper}}`
- `{{user.name | lower}}`
- `{{payload | json}}` or `{{payload | tojson}}`
- `{{user.last_seen | ago}}`

Filtered placeholders fail closed. If the value is missing or the filter is unsupported, the placeholder renders as an empty string instead of leaking raw `{{...}}` text to the user.
