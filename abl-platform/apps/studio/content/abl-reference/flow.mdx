...
Function-style `CALL:` syntax normalizes the lookup key to the bare tool name, so `CALL: check_outage_by_address(service_address)` resolves the registered tool `check_outage_by_address`.
...
Bare dotted identifiers such as `result.user_id` are treated as value-path expressions. Quote the value explicitly if you need to store the literal string `"result.user_id"`.
...
`THEN:` and `ON_FAIL:` can also use the terminal keywords `COMPLETE` and `ESCALATE`. Those terminal targets are case-insensitive.
