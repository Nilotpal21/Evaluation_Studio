...
export interface IArchWorkspaceConfigRecord {
  _id: string;
  tenantId: string;
  modelId: string;
  provider: string;
...
const ArchWorkspaceConfigSchema = new Schema<IArchWorkspaceConfigRecord>(
  {
  ...
  { timestamps: true, collection: 'arch_workspace_configs_v4' },
);
...
export const ArchWorkspaceConfigModel =
  (mongoose.models.ArchWorkspaceConfigModel as any) ||
  model<IArchWorkspaceConfigRecord>('ArchWorkspaceConfigModel', ArchWorkspaceConfigSchema);
