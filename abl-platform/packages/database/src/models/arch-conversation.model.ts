...
export interface IArchConversation {
  _id: string;
  userId: string;
  projectId: string;
...
const ArchConversationSchema = new Schema<IArchConversation>(
  {
  ...
  { timestamps: true, collection: 'arch_conversations' },
);
...
export const ArchConversation =
  (mongoose.models.ArchConversation as any) ||
  model<IArchConversation>('ArchConversation', ArchConversationSchema);
